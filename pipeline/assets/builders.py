...
from .config import W, H, STOCK_CODES, normalize_stock_name, get_stock_sector, BROKERAGE_FIRMS
...
def _build_stock_summary(sec, out_path, img_dir, visual=None):
    ...
    # lower-third(종목명/코드/등락률/섹터)는 .content 하단에 고정 도킹되므로,
    # 그 위 일반 흐름 콘텐츠가 겹치지 않도록 하단 여백을 미리 확보한다.
