...
# LLM 내레이션이 v3-1 원본 소스 필드의 "전일(M/D)" 축약 표기(예: 가격 라벨
# "전일종가(8/19)"의 앞 절반)를 그대로 베껴 쓰는 경우가 있어, TTS가 괄호/
# 슬래시를 그대로 읽어버린다(사용자 피드백). "전일"로 범위를 좁힌 이유:
# 괄호 뒤에 조사가 바로 붙는 경우(예: "전일종가(8/19)는")까지 일반화하면
# "...일는"처럼 문법이 깨지는 부작용이 있는데, 실제로 확인된 문제는
# "전일(M/D)" 단독 사용뿐이라 그 범위로만 한정한다.
_JEONIL_DATE_RE = re.compile(r"전일\((\d{1,2})/(\d{1,2})\)")


def read_jeonil_date_ko(text: str) -> str:
    """텍스트 안의 "전일(M/D)" 패턴을 "전일인 M월 D일"로 치환한다. 예:
    "전일(8/19)" → "전일인 8월 19일"."""
    if not text:
        return text
    return _JEONIL_DATE_RE.sub(
        lambda m: f"전일인 {int(m.group(1))}월 {int(m.group(2))}일", text
    )
