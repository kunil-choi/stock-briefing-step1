...
def _prefix_narration(section: dict, prefix: str) -> dict:
    """섹션 사본의 내레이션/자막 맨 앞에 고정 전환 멘트를 붙인다(원본은 불변).
    화면 렌더러(builders.py)는 narration 텍스트를 그대로 그리는 게 아니라
    섹션을 id로 다시 찾아 화면을 구성하므로, 이 접두어는 내레이션·자막에만
    반영되고 화면 구성에는 영향을 주지 않는다.

    ★ 개별 종목 섹션(stock_.../hidden_..., 집계 섹션 제외)은 generate_voice.py
    의 _build_jobs()가 "narration"이 아니라 "narration_summary"/
    "subtitle_summary"를 TTS 원문으로 읽는다(narration_summary가 있으면
    narration은 아예 안 봄). 여기서 "narration"/"subtitle"에만 접두어를
    붙이면 실제 음성에는 전혀 반영되지 않는 조용한 버그가 생기므로, 어느
    필드가 실제로 낭독되는지 맞춰서 접두어를 붙인다."""
    sid = section.get("id", "")
    is_individual_stock = (
        (sid.startswith("stock_") or sid.startswith("hidden_"))
        and sid not in _AGGREGATE_STOCK_IDS
    )
    field, sub_field = (
        ("narration_summary", "subtitle_summary") if is_individual_stock
        else ("narration", "subtitle")
    )
    narration = (prefix + " " + (section.get(field) or "")).strip()
    subtitle  = (prefix + " " + (section.get(sub_field) or "")).strip()
    return {**section, field: narration, sub_field: subtitle}
...
def _fmt_index_line(name: str, value: str, change: str) -> str:
    if not value:
        return ""
    return f"{name}는 {value}, 전일 대비 {change}로 마감했습니다." if change else f"{name}는 {value}로 마감했습니다."
